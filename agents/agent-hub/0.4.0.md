...
4. Run `npm run finalize:agent-pack -- <path-to-pack>` so the canonical pack validates, the matching Claude-compatible skill is generated, and that skill validates too.
...
6. Open a GitHub PR so the canonical pack, generated skill, prompts, and validation story remain reviewable in one place.
